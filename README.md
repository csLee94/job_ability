...
### Project 목차
1. [분석 주제 및 분석 단위 선정](#1-분석-주제-선정)
2. [가설 설정 및 가설 검정](#2-가설-설정-및-가설-검정)
...
> ![img](https://drive.google.com/uc?id=1CzUPtCzG8MKssq7_9iJKykR58wOp-3PU)

제공되는 데이터의 Column은 학점/토익점수/영어회화/제 2외국어/자격증 개수/인턴 횟수/공모전 수상 횟수/봉사활동/해외 경험으로 총 9가지였다.
위 데이터를 전체 합격자/합격자 중 상위 25%/합격자 중 하위 25%의 분포를 확인하고 각각 평균과 분산이 어떠한지 확인한다.
각 분야 별로 보니 다음과 같은 분포도를 그릴 수 있었다.
![img](https://drive.google.com/uc?id=1ibs27GautVdjB3vE3qE6gkPs9EhJYFJW)

<br>

각 분야들을 종합하여 종합 점수 분포도를 그렸을 때 다음과 같은 분포도를 그릴 수 있었다.
![img](https://drive.google.com/uc?id=1S4BkSV27FXnQ1HePccrG5j1d3WFGW7Nx)

> 1. 상위 25%와 전체 합격자 분포는 거의 차이가 없었다. 전체적으로 스펙의 영향이 크게 미쳤다고 해석할 수 있었다.
> 2. 다만 하위 25%는 위 두 그룹과 확연한 차이를 보였는데, 이는 `스펙 외 유효한 역량이 있음을 유추`할 수 있었다.

## 프로젝트 결론
합격자 데이터의 분포로 봤을 때 아직까지 스펙 합격자 결정에 큰 영향을 미치는 것을 알 수 있었다. 그러나 스펙 없이도 합격한 하위25% 그룹이 합격할 수 있었던 별도의 요인이 있다는 것 역시 확인할 수 있었다. 또한 수시 채용 확대가 현재 진행 중인 과도기라는 점과, 데이터 업데이트가 실제보다 다소 느리다는 점을 토대로 봤을 때, 스펙 외 합격 요인이 유의미하다고 볼 수 있을 것 같다. 이에 더해, 위에서 언급한 HR전문가들의 채용 시장 변화에 대한 생각 역시 설득력이 있다고 볼 수 있겠다.

그렇다면 스펙 외 합격 요인은 무엇일까? 필살기를 준비하라고 언급한 [면접왕이형](https://www.youtube.com/channel/UCp-C7mtkuOw6q8E1Uc2NVpQ)은 다음과 같이 말한다.
> `유사 경험` + `성공 경험` + `인사이트` <br> 직무와 산업을 선택하고, 해당 분야와 관련 있는 경험을 진행하던 중 겪은 성공 사례, 더해서 이를 통해 새로 얻은 인사이트

위와 같은 구조를 완성시키기 위해서는 어떤 `경험`이 내가 선택한 직무에 관련이 있는지를 먼저 알아야 한다. 때문에 각각 직무에서 선호되거나 연관 있는 경험들에 대한 분석이 필요했다.
이를 분석하기 위하여 채용 공고 내에서 [주요 업무/자격 조건/우대사항]이 매우 상세하게 적혀 있는 원티드의 채용공고를 분석 대상으로 선정하였다. 채용 공고를 MYSQL DB에 저장하여 Konlpy 중 Okt를 사용하여 명사와 형용사를 분리하였다. 영어는 별도로 Nltk Library를 이용하여 분리하였다. 
> *okt를 선택한 이유* <br> 1. kkma나 hannanum에 비해 덜 자세히 분리가 되어 사용하였다. `머신러닝`처럼 붙어야할 단어들이 `머신`, `러닝`으로 분류되는 경우를 방지하고자 했다. <br> 2. 말뭉치 업데이트가 수월했다. 자격증 이름이나, 기존 말뭉치에 등록되어 있지 않은 단어들을 쉽게 jar 파일을 통해 업데이트 할 수 있었다.

이 과정을 통해 나누어진 단어들을 직군 별로 축적해나갔다. 등록된 채용 공고를 전수조사 하여 약 2만개의 채용 공고를 수집하였고, 이를 분리하여 약 110만 개의 단어를 추출하였다. 준비하고 있는 **데이터 분석가** 직무에서 많이 언급된 순서로 확인해보니 데이터 분석 경험, 서비스, 기획 등 경험 관련 단어부터 Python, SQL, GA, Tableau 등 툴 관련 단어들까지 확인할 수 있었다. 

<br>

# 3.서비스 기획 및 개발
수집한 자료와 분석 결과를 이용하여 현재 구직자들에게 도움을 줄 수 있는 서비스를 기획하였다. 채용 공고 속 빈출되는 단어들을 통해 직무별 역량 육성 우선순위를 정할 수 있을 것이고, 새로운 수시 채용 시장에서 `필살기`를 정하는데도 효율적일 것이다.
